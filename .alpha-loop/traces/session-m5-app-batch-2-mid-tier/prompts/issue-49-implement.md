Implement GitHub issue #49: Daily Updates: app-specific tests

## Summary
Add test coverage for Daily Updates — update aggregation and category filtering.

## Acceptance Criteria
- [ ] Test: update feed renders with mock data
- [ ] Test: category filtering works
- [ ] Test: profile badges display correctly
- [ ] Test: auth gate redirects work
- [ ] `pnpm test` passes


## Implementation Plan
1. Create apps/web/app/apps/daily-updates/components/__tests__/feed-app.test.tsx:
   - Mock @repo/ui (Tabs with context, Card, CardHeader, CardContent, CardFooter, Badge, Avatar, AvatarFallback, Button, Input, EmptyState).
   - Build mock DailyUpdate[] fixtures with varied source_app, category, priority, links, and reactions.
   - Build mock AppProfile[] and categories[] fixtures.
   - Test: feed renders UpdateCards for each mock update (title, body, source_name, source_icon visible).
   - Test: profile badges display correctly — source icon in avatar, source_name in neon color, category badge shows formatted name, priority badge shows fire emoji for high priority.
   - Test: category filter dropdown appears when categories provided; changing it filters updates to matching category only.
   - Test: app filter dropdown filters by source_app.
   - Test: search input filters by title/body/source_name.
   - Test: empty state renders when no updates match filters.
   - Test: time range tab triggers render (All Time, Today, This Week, This Month).
   - Test: reaction button click increments count.
   - Test: Load more button renders when hasMore is true (initialUpdates.length === 20).

2. Create apps/web/app/apps/daily-updates/__tests__/layout.test.tsx:
   - Mock @/lib/require-app-layout-access.
   - Test: calls requireAppLayoutAccess with 'daily-updates'.
   - Test: renders children when authorized.
   - Test: throws when requireAppLayoutAccess rejects (unauthorized redirect).
   - Test: renders header with 'Daily Updates' title and phone emoji.
   - Test: renders nav links (Feed, About).

3. Run `pnpm test` from apps/web to verify all tests pass.
...
- Issue #48: Daily Updates: component migration
- Status: success
- Tests: PASSING
- Files changed: 3
- Duration: 424s
- PR: https://github.com/last-rev-llc/lr-apps/pull/137
...
3. Commit with: git commit -m "feat: Daily Updates: app-specific tests (closes #49)"