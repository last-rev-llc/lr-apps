...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  <div class="hero">
    ${avatarUrl ? `<img class="avatar" src="${esc(avatarUrl)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="avatar-placeholder" style="display:none">${initials}</div>` : `<div class="avatar-placeholder">${initials}</div>`}
    <div class="hero-info">
  ...
  _formatMsg(text, max) {
    return window.CCSlack ? CCSlack.format(text, max || 200) : (text || '').substring(0, 200);
  }
  ...
  _socialIconsHtml(user, inline) {
    ...
    const links = user.socials.map(s =>
      `<a href="${this._esc(s.url)}" target="_blank" rel="noopener" class="user-social-link" title="${this._esc(s.platform + ': ' + s.handle)}">${this._socialIcon(s.platform)}</a>`
    ).join('');
  ...
  _socialTabHtml(user) {
    ...
    socials.forEach(s => {
      ...
      html += `<div class="social-feed-section">
        <div class="social-feed-header">
          ...
          <a href="${this._esc(s.url)}" target="_blank" rel="noopener" class="social-feed-link">@${this._esc(s.handle)} ↗</a>
        </div>`;

      if (posts.length) {
        html += posts.map(p => `<div class="card social-post">
          <div class="social-post-text">${this._esc((p.text || '').slice(0, 280))}${(p.text || '').length > 280 ? '…' : ''}</div>
          ${p.media ? `<div class="social-post-media"><img src="${this._esc(p.media)}" onerror="this.style.display='none'"></div>` : ''}
          <div class="social-post-footer">
            ...
            ${p.url ? `<a href="${this._esc(p.url)}" target="_blank" rel="noopener" class="social-post-link">View post ↗</a>` : ''}
          </div>
        </div>`).join('');
      } else {
        html += `<div class="no-data-box">
          <div class="no-data-box-msg">No cached posts yet</div>
          <a href="${this._esc(s.url)}" target="_blank" rel="noopener" class="btn btn-sm btn-icon" title="View ${this._esc(platformLabel)} Profile"><i data-lucide="external-link" style="width:14px;height:14px"></i></a>
        </div>`;
      }
    ...
    html += `<div style="text-align:center;padding:8px 0">
      <button class="btn-refresh" onclick="this.closest('cc-users')._fetchSocialFeeds('${user.id}')" title="Refresh social feeds"><i data-lucide="refresh-cw"></i></button>
    ...
    return `<div class="profile-summary">
      <div class="profile-summary-row">
        ...
        <div class="action-row">
          <button class="btn btn-sm btn-icon" onclick="event.stopPropagation();this.closest('cc-users')._runFullRefresh('${user.id}')" title="Research &amp; analyze this person"><i data-lucide="refresh-cw" style="width:14px;height:14px"></i></button>
        </div>
      </div>
  ...
  _runResearch(userId) {
    ...
    CCTrigger(`Run full people research on: ${user.name} (${ctx}). Use the people-research skill. Search Slack messages, GitHub activity, web/LinkedIn. Update their entry in the contacts table with any new info found (title, phone, timezone, socials, companies, avatar, tags, notes). Merge with existing data, don't overwrite good data with blanks.`);
  ...
  _runAnalysis(userId) {
    const user = this._users.find(u => u.id === userId);
    if (!user) return;
    CCTrigger(`Run personality insights analysis on: ${user.name} (Slack ID: ${user.slackId || 'unknown'}, GitHub: ${user.githubHandle || 'unknown'}). Use the personality-insights skill. Gather their Slack messages, GitHub activity, and any social data. Generate communication style profile, interests, conversation starters, best approach, topics to avoid. Save the insights object to their entry in the contacts table under the "insights" key. If they already have insights, merge and refresh — don't lose existing good data.`);
  ...
  _runFullRefresh(userId) {
    ...
    CCTrigger(`Run full people research AND personality insights for: ${user.name} (${ctx}). First, use the people-research skill to search Slack messages, GitHub activity, web/LinkedIn and update their entry in the contacts table with any new info found. Then, use the personality-insights skill to generate/refresh their communication style profile, interests, conversation starters, best approach, and topics to avoid — save under the "insights" key in the contacts table. Merge with existing data, don't overwrite good data with blanks.`);
    ...
    if (handle) {
      ...
      html += `<div class="social-feed-header" style="margin-bottom:12px">
        ...
        <div class="flex gap-2 align-center">
          <a href="https://github.com/${this._esc(handle)}?tab=repositories" target="_blank" class="social-feed-link">View all ↗</a>
          <button class="btn-refresh" onclick="event.stopPropagation();this.closest('cc-users')._fetchGithubRepos('${this._esc(handle)}')" title="Refresh repos"><i data-lucide="refresh-cw"></i></button>
        </div>
      </div>`;
      ...
      if (repos.length) {
        html += repos.map(r => {
          ...
          return `<div class="card repo-card">
            <div class="repo-card-header">
              <a href="${this._esc(r.url || `https://github.com/${handle}/${r.name}`)}" target="_blank" class="repo-card-name">${this._esc(r.name)}</a>
        ...
        html += `<div class="no-data-box" style="margin-bottom:16px">
          ...
          <button class="btn-refresh" onclick="event.stopPropagation();this.closest('cc-users')._fetchGithubRepos('${this._esc(handle)}')" title="Fetch repos"><i data-lucide="refresh-cw"></i></button>
        </div>`;
      }
    }
  ...
  _socialDetailHtml(user) {
    if (!user.socials || !user.socials.length) return '';
    const links = user.socials.map(s =>
      `<div class="detail-item">${this._socialIcon(s.platform)} <a href="${this._esc(s.url)}" target="_blank" >${this._esc(s.platform)} — ${this._esc(s.handle)}</a></div>`
    ).join('');
  ...
  _avatarHtml(u, size) {
    ...
    if (u.avatar) {
      return `<img src="${this._esc(u.avatar)}" alt="" class="avatar" style="width:${sz}px;height:${sz}px" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'"><div class="avatar-initials" style="width:${sz}px;height:${sz}px;font-size:${fs}px;display:none">${this._getInitials(u.name)}</div>`;
    }
  ...
  _renderCompactRows(users) {
    ...
    const rows = users.map(u => {
      ...
      return `<div class="user-compact-row" onclick="this.closest('cc-users')._openUser('${u.id}')">
  ...
  _renderCardGrid(users) {
    ...
    return `<div class="user-grid">${users.map(u => {
      ...
      return `<div class="card user-card" onclick="this.closest('cc-users')._openUser('${u.id}')">
        ...
        <div class="user-card-links">
          ${u.slackHandle ? `<a href="https://lastrev.slack.com/team/${this._esc(u.slackId || '')}" target="_blank" style="text-decoration:none"><span class="badge badge-icon">${this._brandIcon('slack',12)} @${this._esc(u.slackHandle)}</span></a>` : ''}
          ${u.githubHandle ? `<a href="https://github.com/${this._esc(u.githubHandle)}" target="_blank" style="text-decoration:none"><span class="badge badge-icon">${this._brandIcon('github',12)} ${this._esc(u.githubHandle)}</span></a>` : ''}
  ...
  _render() {
    ...
    const counts = { all: this._users.length, team: 0, client: 0, lead: 0, partner: 0, contractor: 0, personal: 0, other: 0 };
    this._users.forEach(u => { if (counts[u.type] !== undefined) counts[u.type]++; });

    const typeItems = ['all', 'team', 'client', 'lead', 'partner', 'contractor', 'personal', 'other'].map(t => ({
    ...
    const viewToggle = `<div class="view-toggle">
      <button class="view-btn ${isCards ? 'active' : ''}" onclick="this.closest('cc-users')._setView('cards')" title="Card view"><i data-lucide="grid-2x2" style="width:16px;height:16px"></i></button>
      <button class="view-btn ${!isCards ? 'active' : ''}" onclick="this.closest('cc-users')._setView('compact')" title="List view"><i data-lucide="list" style="width:16px;height:16px"></i></button>
    </div>`;
        ...
        <div class="scrollable-body" style="max-height:300px">
          ${pendingMatches.map((p, i) => {
            ...
            return `<div class="entity-row">
              <div class="flex gap-2 align-center">
                ${slackImg ? `<img src="${this._esc(slackImg)}" class="avatar" style="width:36px;height:36px">` : `<div class="user-avatar" style="width:36px;height:36px;font-size:13px">${this._getInitials(p.userName)}</div>`}
                <span style="font-size:18px">↔</span>
                ${matchImg ? `<img src="${this._esc(matchImg)}" class="avatar" style="width:36px;height:36px">` : `<div class="avatar-initials" style="width:36px;height:36px;font-size:14px">${platformIcons[p.match.platform] || '?'}</div>`}
              </div>
              <div class="entity-body">
                <div class="person-name">${this._esc(p.userName)}</div>
                <div class="entity-meta">${platformIcons[p.match.platform] || ''} ${this._esc(p.match.platform)} — <a href="${this._esc(p.match.url)}" target="_blank" style="color:var(--accent)">${this._esc(p.match.handle || p.match.profileName || '')}</a></div>
              ...
              <div class="entity-actions">
                ...
                <button class="btn btn-sm btn-icon btn-approve" onclick="event.stopPropagation();this.closest('cc-users')._approveMatch(${i})" title="Approve match"><i data-lucide="check" style="width:14px;height:14px"></i></button>
                <button class="btn btn-sm btn-icon btn-reject" onclick="event.stopPropagation();this.closest('cc-users')._rejectMatch(${i})" title="Reject match"><i data-lucide="x" style="width:14px;height:14px"></i></button>
              </div>
            </div>`;
          }).join('')}
        </div>
    ...
    if (expandedUser) {
      ...
      const overviewContent = `
          ...
          <div class="detail-meta" style="margin-top:16px">
            ...
            ${expandedUser.slackHandle ? `<div class="detail-item">${this._brandIcon('slack',14)} <a href="https://lastrev.slack.com/team/${this._esc(expandedUser.slackId || '')}" target="_blank">@${this._esc(expandedUser.slackHandle)}</a></div>` : ''}
            ${expandedUser.githubHandle ? `<div class="detail-item">${this._brandIcon('github',14)} <a href="https://github.com/${expandedUser.githubHandle}" target="_blank">${this._esc(expandedUser.githubHandle)}</a></div>` : ''}
          ...
          ${(expandedUser.tags || []).length ? `<div class="tags-row">${expandedUser.tags.map(t => `<span class="badge">${t}</span>`).join('')}</div>` : ''}
          <div class="field-group">
            <div class="field-item">
              <label class="field-label">Alias Name</label>
              <input type="text" value="${this._esc(aliasVal)}" placeholder="e.g. Max Techera"
                onchange="this.closest('cc-users')._saveUserField('${expandedUser.id}','alias',this.value)"
                class="input">
            </div>
            <div class="field-item">
              <label class="field-label">Nickname</label>
              <input type="text" value="${this._esc(nicknameVal)}" placeholder="e.g. Max"
                onchange="this.closest('cc-users')._saveUserField('${expandedUser.id}','nickname',this.value)"
                class="input">
            </div>
          </div>
          <div style="margin-top:12px">
            ...
            <textarea rows="3" placeholder="Your private notes about this person..."
              onchange="this.closest('cc-users')._saveUserField('${expandedUser.id}','myNotes',this.value)"
              class="textarea-notes">${this._esc(myNotes)}</textarea>
          </div>
      ...
      modalHtml = `<cc-modal title="${this._esc(expandedUser.name)}" size="lg" open>
        <div class="user-detail">
          ...
          <div class="user-detail-header" style="margin-top:16px">
            ...
            <div>
              <h2 style="margin:0;font-size:20px;color:var(--text)">${this._esc(expandedUser.name)} ${this._getTypeBadge(expandedUser.type)} <button class="btn btn-sm btn-icon" onclick="event.stopPropagation();this.closest('cc-users')._downloadPdf('${expandedUser.id}')" title="Download Profile PDF" style="margin-left:8px;vertical-align:middle"><i data-lucide="file-down" style="width:14px;height:14px"></i></button></h2>
    ...
    this.innerHTML = `
      <div class="page-header">
        <h1>👥 <span>Team Directory</span></h1>
        <div class="flex align-center gap-3">
          ${viewToggle}
          <span class="badge">${users.length} people</span>
        </div>
      </div>
      <div class="filters" style="flex-direction:column;gap:12px;align-items:flex-start">
        <div class="filter-row">
          <span class="filter-label">Search</span>
          <cc-search placeholder="Search name, email, company, title..." value="${this._esc(this._search)}" input-style="flex:1;"></cc-search>
        </div>
        <cc-pill-filter label="Type" items='${JSON.stringify(typeItems)}' value="${this._typeFilter}"></cc-pill-filter>
        <cc-pill-dropdown label="Company" items='${JSON.stringify(companyItems)}' value="${this._companyFilter}"></cc-pill-dropdown>
      </div>
      ${pendingHtml}
      ${contentHtml}
      ${modalHtml}`;
    setTimeout(() => {
      ...
      const pillFilter = this.querySelector('cc-pill-filter[label="Type"]');
      if (pillFilter) pillFilter.addEventListener('pill-change', (e) => this._setType(e.detail.value));
      const companyDd = this.querySelector('cc-pill-dropdown[label="Company"]');
      if (companyDd) companyDd.addEventListener('pill-change', (e) => this._setCompany(e.detail.value));
