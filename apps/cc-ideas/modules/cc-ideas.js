...
class CcIdeas extends HTMLElement {
  ...
  _initDelegation() {
    this.addEventListener('click', e => {
      ...
      e.stopPropagation();
      switch (action) {
        case 'rate': this._rate(id, parseInt(el.dataset.stars, 10)); break;
        case 'snooze': this._showSnoozeMenu(id, el); break;
        case 'complete': this._toggleComplete(id); break;
        case 'hide': this._toggleHide(id); break;
        case 'copy': this._copyIdea(id); break;
        case 'build': this._buildIdea(id); break;
        case 'build-prompt': this._buildIdeaWithPrompt(id); break;
        case 'save-prompt': this._saveIdeaPrompt(id); break;
        case 'detail': this._openIdeaDetail(id); break;
        case 'quick-filter': this._applyQuickFilter(parseInt(el.dataset.idx, 10)); break;
        case 'generate': this._generateIdeas(); break;
        case 'add': this._openAddModal(); break;
        case 'submit': this._submitIdea(); break;
      }
    });
  }
  ...
  _hasActiveFilters() {
    return this._activeCat !== 'All' || this._activeSource !== 'All' || this._activeApp !== 'All' ||
      this._activeShow !== 'active' || this._activeSort !== 'rating' || this._sortAsc ||
  ...
  _render() {
    ...
    this.innerHTML = `
      <cc-page-header icon="💡" title="Ideas" description="Feature ideas & backlog" count="${filtered.length}" count-label="idea${filtered.length !== 1 ? 's' : ''}">
          <button class="btn-add" data-action="add" title="Add Idea"><i data-lucide="plus"></i></button>
          <button class="btn-refresh" data-action="generate" id="gen-btn" title="Generate Ideas"><i data-lucide="refresh-cw"></i></button>
      </cc-page-header>
      ...
      <div style="margin-bottom:10px;">
        <div style="display:flex;gap:12px;align-items:center;">
          <cc-search placeholder="Search ideas, tags, integrations…" value="${this._escAttr(this._searchQuery)}" input-style="width:100%;"></cc-search>
          <cc-view-toggle app="ideas" value="${this._viewMode}"></cc-view-toggle>
          <cc-filter-drawer title="Filters" ${this._hasActiveFilters() ? 'active' : ''}>
            <div class="filter-section-label" style="font-size:11px;text-transform:uppercase;letter-spacing:0.5px;color:var(--muted);margin-bottom:2px;">Quick Filters</div>
            <div style="display:flex;gap:6px;flex-wrap:wrap;">
              ${this._quickFilters.map((f, i) => `<button class="pill${this._activeQuickFilter === i ? ' active' : ''}" data-action="quick-filter" data-idx="${i}">${f.label}</button>`).join('')}
            </div>
            <div class="filter-section-label" style="font-size:11px;text-transform:uppercase;letter-spacing:0.5px;color:var(--muted);margin:8px 0 2px;">Sort &amp; Filter</div>
            ...
            <cc-pill-dropdown label="Category" items='${JSON.stringify(['All',...CATEGORIES.slice(1)])}' value="${this._escAttr(this._activeCat)}"></cc-pill-dropdown>
            <cc-pill-dropdown label="Source" items='${JSON.stringify(SOURCES.map(s => s === 'All' ? 'All' : s.charAt(0).toUpperCase() + s.slice(1)))}' value="${this._escAttr(this._activeSource === 'All' ? 'All' : this._activeSource.charAt(0).toUpperCase() + this._activeSource.slice(1))}"></cc-pill-dropdown>
            ${!this._appAttr && (this._appList||[]).length ? `<cc-pill-dropdown label="App" items='${this._escAttr(JSON.stringify(["All",...(this._appList||[])]))}' value="${this._escAttr(this._activeApp)}"></cc-pill-dropdown>` : ''}
            <cc-pill-dropdown label="Show" items='${this._escAttr(JSON.stringify([
              ...
              {value:'all',label:'All'}
            ]))}' value="${this._escAttr(this._activeShow)}"></cc-pill-dropdown>
          </cc-filter-drawer>
        </div>
      </div>
      ${this._viewMode === 'list' ? `<div class="view-list">${filtered.map(x => {
        const isCompleted = x.status === 'completed' || completed[x.id];
        return `<div class="list-row" data-action="detail" data-id="${this._escAttr(x.id)}">
      ...
      : this._viewMode === 'expanded' ? `<div class="view-expanded">${filtered.map(x => {
        const isCompleted = x.status === 'completed' || completed[x.id];
        return `<div class="expanded-card" data-action="detail" data-id="${this._escAttr(x.id)}">
      ...
      : `<div class="idea-grid">${filtered.map(x => {
        ...
        return `
        <div class="idea-card${isHidden ? ' hidden-card' : ''}" id="card-${this._escAttr(x.id)}" data-action="detail" data-id="${this._escAttr(x.id)}">
          ...
          ${x.author ? `<div style="font-size:11px;color:var(--muted);margin-top:4px;">by <a href="${this._escAttr(x.sourceUrl||'#')}" target="_blank" rel="noopener" style="color:var(--accent);" onclick="event.stopPropagation()">${esc(x.author)}</a></div>` : ''}
          ...
          <div style="display:flex;align-items:center;margin-top:10px;">
            ...
            <div style="display:flex;gap:6px;align-items:center;" onclick="event.stopPropagation()">
              <div class="stars">${[1,2,3,4,5].map(s => `<span class="star${s <= (ratings[x.id]||0) ? ' on' : ''}" data-action="rate" data-id="${this._escAttr(x.id)}" data-stars="${s}"><i data-lucide="star"${s <= (ratings[x.id]||0) ? ' style="fill:currentColor"' : ''}></i></span>`).join('')}</div>
              <button class="snooze-btn${this._isSnoozed(x.id) ? ' snoozed' : ''}" title="${this._escAttr(this._isSnoozed(x.id) ? 'Snoozed until ' + new Date(snoozed[x.id]).toLocaleDateString() : 'Snooze')}" data-action="snooze" data-id="${this._escAttr(x.id)}"><i data-lucide="alarm-clock" style="width:14px;height:14px;"></i></button>
              <button class="complete-btn" title="${isCompleted ? 'Undo complete' : 'Mark complete'}" data-action="complete" data-id="${this._escAttr(x.id)}">${isCompleted ? '<i data-lucide="check-circle" style="width:14px;height:14px;"></i>' : '<i data-lucide="check" style="width:14px;height:14px;"></i>'}</button>
              <button class="hide-btn" title="${isHidden ? 'Restore' : 'Dismiss'}" data-action="hide" data-id="${this._escAttr(x.id)}">${isHidden ? '<i data-lucide="eye"></i>' : '<i data-lucide="x"></i>'}</button>
            </div>
          </div>
          <div class="card-detail" style="display:none;">
            ...
            ${(x.resources||[]).length ? `<div class="detail-label">Resources</div><div class="detail-text">${(x.resources||[]).map(r => `<a href="${this._escAttr(r)}" target="_blank" rel="noopener" style="color:var(--accent);font-size:12px;display:block;" onclick="event.stopPropagation()">${esc(r)}</a>`).join('')}</div>` : ''}
            <div style="margin-top:8px;" onclick="event.stopPropagation()">
              <button class="copy-btn" data-action="copy" data-id="${this._escAttr(x.id)}"><i data-lucide="clipboard"></i> Copy</button>
              <button class="build-btn" style="background:transparent;color:var(--green);border:1px solid var(--green);border-radius:6px;padding:4px 12px;cursor:pointer;font-size:12px;font-weight:600;margin-left:8px;" data-action="build" data-id="${this._escAttr(x.id)}"><i data-lucide="hammer"></i> Build It</button>
            </div>
          </div>
        </div>`;
      }).join('')}</div>`}
...
<cc-modal id="add-idea-modal" title="💡 Add New Idea" size="sm">
  ...
  <div slot="footer">
    ...
    <button class="btn btn-primary btn-sm" data-action="submit">Submit</button>
  </div>
</cc-modal>`;
  ...
  _openIdeaDetail(id) {
    ...
    body.innerHTML = `
      <div style="display:flex;gap:8px;align-items:center;margin-bottom:16px;flex-wrap:wrap;">
        ...
        ${x.author ? `<span style="font-size:12px;color:var(--muted);">by <a href="${this._escAttr(x.sourceUrl||'#')}" target="_blank" style="color:var(--accent);">${esc(x.author)}</a></span>` : ''}
      </div>
      ...
      ${(x.resources||[]).length ? `<div style="margin-bottom:12px;"><h4 style="color:var(--accent);font-size:0.85rem;text-transform:uppercase;letter-spacing:1px;margin-bottom:6px;">Resources</h4>${(x.resources||[]).map(r => `<a href="${this._escAttr(r)}" target="_blank" style="color:var(--accent);font-size:12px;display:block;margin-bottom:2px;">${esc(r)}</a>`).join('')}</div>` : ''}
      ...
      <div style="margin-bottom:16px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          ...
          <div style="display:flex;gap:6px;">
            <button class="btn btn-sm" data-action="save-prompt" data-id="${this._escAttr(x.id)}"><i data-lucide="save"></i> Save</button>
          </div>
        </div>
      ...
      <div style="display:flex;gap:8px;justify-content:flex-end;padding-top:12px;border-top:1px solid var(--border);">
        <button class="btn btn-sm" data-action="copy" data-id="${this._escAttr(x.id)}"><i data-lucide="clipboard"></i> Copy</button>
        <button class="btn btn-sm" style="color:var(--green);border-color:var(--green);" data-action="build-prompt" data-id="${this._escAttr(x.id)}"><i data-lucide="hammer"></i> Build It</button>
      </div>
