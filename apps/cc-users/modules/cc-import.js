...
class CcImport extends HTMLElement {
  ...
  async _handleFile(file) {
    ...
    if (!this._parsed.length) {
      window.showToast && window.showToast('No records found in file', 3000);
      return;
    }
  ...
  _renderMap() {
    const rows = this._sourceFields.map(sf => `
      <tr>
        <td style="padding:8px 12px;font-weight:500;">${this._esc(sf)}</td>
        <td style="padding:8px 12px;color:var(--muted);max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${this._esc(this._parsed[0]?.[sf] || '—')}</td>
        <td style="padding:8px 12px;">
          <select data-source="${this._escAttr(sf)}" style="background:var(--surface);color:var(--text);border:1px solid var(--border);border-radius:8px;padding:6px 10px;width:100%;">
            <option value="">— skip —</option>
            ${this._targetFields.map(tf => `<option value="${this._escAttr(tf)}" ${this._mapping[sf]===tf?'selected':''}>${this._esc(tf)}</option>`).join('')}
          </select>
        </td>
      </tr>`).join('');
    ...
    this.innerHTML = `
      ...
      ${this._combineNames ? `<div class="card" style="margin-bottom:16px;padding:12px 16px;background:rgba(99,102,241,0.08);border:1px solid var(--accent);">
        ✨ Auto-combining <strong>${this._esc(this._nameFirstField)}</strong> + <strong>${this._esc(this._nameLastField)}</strong> → <strong>name</strong>
      </div>` : ''}
  ...
  async _renderPreview() {
    // Check for duplicates against existing users in Supabase
    let existing = [];
    if (window.supabase) {
      try { existing = await window.supabase.select('users', { select: 'email' }); } catch(e) {}
    }
    ...
    const previewRows = preview.map(c => `
      <tr style="${c._isDupe ? 'opacity:0.5;text-decoration:line-through;' : ''}">
        ...
        <td style="padding:6px 10px;">${this._esc(c.name) || '—'}</td>
        <td style="padding:6px 10px;">${this._esc(c.email) || '—'}</td>
        <td style="padding:6px 10px;">${this._esc(c.company) || '—'}</td>
        <td style="padding:6px 10px;">${this._esc(c.title) || '—'}</td>
        <td style="padding:6px 10px;">${this._esc(c.phone) || '—'}</td>
      </tr>`).join('');
  ...
  async _doImport() {
    ...
    try {
      let imported = 0;
      // Batch upsert to Supabase users table
      const batch = toImport.map(c => { delete c._isDupe; delete c.raw_data; return c; });
      if (window.supabase) {
        const BATCH_SIZE = 50;
        for (let i = 0; i < batch.length; i += BATCH_SIZE) {
          const chunk = batch.slice(i, i + BATCH_SIZE);
          await window.supabase.upsert('users', chunk);
      ...
      this._step = 'done';
      this._render();
      window.showToast && window.showToast(`✅ Imported ${imported} contacts!`, 3000);
    } catch(e) {
      console.error('Import failed:', e);
      window.showToast && window.showToast('Import failed: ' + e.message, 5000);
      btn.disabled = false;
