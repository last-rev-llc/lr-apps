...
class CcLeads extends HTMLElement {
  connectedCallback() {
    ...
    this.addEventListener('pill-change', e => { this._setFilter(e.detail.value); });
    this.addEventListener('dropdown-change', e => { this._setSort(e.detail.value); });
  ...
  _render() {
    ...
    if (meetings.length) {
      const rows = meetings.map(m => {
        ...
        const coLinks = (m.companies || []).map(d => `<a href="#co-${this._escAttr(d)}" style="color:var(--accent);text-decoration:none;font-size:12px;">${esc(this._data.companies[d]?.name || d)}</a>`).join(', ');
    ...
    const controlsHtml = `<div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;margin-bottom:20px;">
      <cc-search placeholder="Search companies, people, domains…" value="${this._escAttr(this._search)}" style="flex:1;min-width:200px;"></cc-search>
      <cc-view-toggle app="leads" value="${this._escAttr(this._viewMode)}"></cc-view-toggle>
      <cc-filter-drawer title="Filters" ${this._filter !== 'all' || this._sort !== 'score' || this._sortDir !== 'desc' ? 'active' : ''}>
        <cc-pill-filter label="Fit" items='${this._escAttr(filterItems)}' value="${this._escAttr(this._filter)}"></cc-pill-filter>
        <cc-pill-dropdown label="Sort" items='${this._escAttr(sortItems)}' value="${this._escAttr(this._sort)}" direction="${this._escAttr(this._sortDir)}"></cc-pill-dropdown>
      </cc-filter-drawer>
    </div>`;
    ...
    const cardsHtml = companies.map(c => {
      ...
      const peopleRows = people.map(p => {
        ...
        const topicsHtml = (p.suggestedTopics || []).length ? `
          <div data-expand-key="${this._escAttr(topicsKey)}" onclick="event.stopPropagation();this.closest('cc-leads')._toggleExpand(this.dataset.expandKey)" style="cursor:pointer;font-size:11px;color:var(--muted);margin-top:2px;">
        ...
        return `<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid rgba(42,45,58,0.5);">
          ...
          ${p.linkedinUrl ? `<a href="${this._escAttr(p.linkedinUrl)}" target="_blank" rel="noopener" style="color:#0a66c2;flex-shrink:0;" title="LinkedIn">🔗</a>` : ''}
        </div>`;
      }).join('');
      ...
      const tpHtml = talkingPoints.length ? `
        <div data-expand-key="${this._escAttr(tpKey)}" onclick="this.closest('cc-leads')._toggleExpand(this.dataset.expandKey)" style="cursor:pointer;display:flex;align-items:center;gap:4px;margin-top:8px;font-size:12px;color:var(--muted);">
          ${tpExpanded ? '▼' : '▶'} Talking Points (${talkingPoints.length})
        </div>
      ...
      const reasonsHtml = reasons.length ? `
        <div data-expand-key="${this._escAttr(reasonsKey)}" onclick="this.closest('cc-leads')._toggleExpand(this.dataset.expandKey)" style="cursor:pointer;display:flex;align-items:center;gap:4px;margin-top:4px;font-size:12px;color:var(--muted);">
          ${reasonsExpanded ? '▼' : '▶'} Fit Reasons (${reasons.length})
        </div>
      ...
      const newsHtml = (c.recentNews || []).length ? `
        <div style="margin-top:8px;display:flex;flex-direction:column;gap:2px;">
          ${c.recentNews.map(n => `<a href="${this._escAttr(n.url)}" target="_blank" rel="noopener" style="font-size:11px;color:var(--blue);text-decoration:none;display:flex;align-items:center;gap:4px;">
      ...
      // Social links
      const socials = [];
      if (c.socialLinks?.linkedin) socials.push(`<a href="${this._escAttr(c.socialLinks.linkedin)}" target="_blank" rel="noopener" style="color:#0a66c2;">🔗</a>`);
      if (c.socialLinks?.twitter) socials.push(`<a href="${this._escAttr(c.socialLinks.twitter)}" target="_blank" rel="noopener" style="color:#1d9bf0;">🐦</a>`);
      const socialsHtml = socials.length ? `<div style="display:flex;gap:8px;align-items:center;">${socials.join('')}</div>` : '';

      return `<div id="co-${this._escAttr(c.domain)}" class="panel" style="margin-bottom:16px;">
        <div style="display:flex;align-items:flex-start;gap:12px;margin-bottom:12px;">
          ...
          <div style="flex:1;min-width:0;">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
              ...
              <a href="https://${this._escAttr(c.domain)}" target="_blank" rel="noopener" style="font-size:12px;color:var(--accent);text-decoration:none;">${esc(c.domain)} ↗</a>
              ${socialsHtml}
            </div>
    ...
    this.innerHTML = `
      <cc-page-header icon="🎯" title="Lead Research" description="Sales pipeline & prospect tracking">
      ...
      ${meetingsHtml}
      ${controlsHtml}
      ${this._viewMode === 'list' ? (companies.length ? `<div class="view-list">${companies.map(c => {
        const fit = this._fitColor(c.lastRevFit.score);
        return `<div class="list-row">
          <span style="font-size:20px;font-weight:800;color:${fit.text};flex-shrink:0;min-width:28px;text-align:center;">${c.lastRevFit.score}</span>
          <span class="row-name">${esc(c.name)}</span>
          <span style="font-size:12px;color:var(--muted);flex-shrink:0;">${esc(c.domain)}</span>
          <span class="row-desc">${esc(c.lastRevFit.reasoning || '')}</span>
          <span class="row-tags">${[c.techStack.cms, c.techStack.framework].filter(Boolean).map(t => `<span class="tag">${esc(t)}</span>`).join('')}</span>
        </div>`;
      }).join('')}</div>` : '<cc-empty-state message="No leads match your search" icon="🔍" animation="none"></cc-empty-state>')
      : this._viewMode === 'expanded' ? (companies.length ? `<div class="view-expanded">${companies.map(c => {
        const fit = this._fitColor(c.lastRevFit.score);
        const allTech = [c.techStack.cms, c.techStack.framework, c.techStack.hosting, ...(c.techStack.other || [])].filter(Boolean);
        const people = this._getPeopleFor(c.domain);
        return `<div class="expanded-card">
          <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px;">
            <span style="font-size:24px;font-weight:800;color:${fit.text};">${c.lastRevFit.score}</span>
            <span style="font-size:16px;font-weight:700;">${esc(c.name)}</span>
            <span style="font-size:13px;color:var(--muted);">${esc(c.domain)}</span>
          </div>
          <div style="color:var(--text);line-height:1.6;margin-bottom:10px;">${esc(c.lastRevFit.reasoning || '')}</div>
          <div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px;">
            ${allTech.map(t => `<span class="tag">${esc(t)}</span>`).join('')}
          </div>
          ${people.length ? `<div style="font-size:12px;color:var(--muted);">Contacts: ${people.map(p => esc(p.name)).join(', ')}</div>` : ''}
        </div>`;
      }).join('')}</div>` : '<cc-empty-state message="No leads match your search" icon="🔍" animation="none"></cc-empty-state>')
