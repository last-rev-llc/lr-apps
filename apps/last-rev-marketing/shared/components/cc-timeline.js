(function () {
  ...
  class CcTimeline extends HTMLElement {
    static get observedAttributes() { return ['steps', 'variant', 'numbered', 'center']; }
    ...
    _render() {
      ...
      let variant = this.getAttribute('variant') || 'vertical';
      const numbered = this.getAttribute('numbered') !== 'false';
      const center = this.hasAttribute('center') || variant === 'alternating';
      if (center && variant === 'vertical') variant = 'alternating';

      this.setAttribute('data-variant', variant);

      if (variant === 'alternating') {
        this.innerHTML = `<div class="cc-timeline__track cc-timeline__track--alternating">
          ${steps.map((s, i) => `
            <div class="cc-timeline__step cc-timeline__step--${i % 2 === 0 ? 'left' : 'right'}" data-step="${i}">
              <div class="cc-timeline__content">
              ...
              <div class="cc-timeline__spacer"></div>
            </div>
          `).join('')}
        </div>`;
      } else {
    ...
    _injectStyles() {
      ...
      s.textContent = `
        ...
        .cc-timeline__step { position: relative; padding-bottom: 48px; opacity: 0; transform: translateY(16px); transition: opacity 0.4s ease, transform 0.4s ease; }
        ...
        .cc-timeline__track--alternating .cc-timeline__step { display: grid; grid-template-columns: 1fr auto 1fr; gap: 24px; align-items: center; padding-bottom: 48px; opacity: 0; transform: translateY(16px); transition: opacity 0.4s ease, transform 0.4s ease; }
        .cc-timeline__track--alternating .cc-timeline__step--visible { opacity: 1; transform: translateY(0); }
