      ...
      this.innerHTML = `
        <div class="cc-cta cc-cta--${_esc(variant)}" style="text-align:${_esc(align)}">
          ...
          <div class="cc-cta__actions" style="justify-content:${align === 'left' ? 'flex-start' : align === 'right' ? 'flex-end' : 'center'}">
            ${pt ? `<a class="cc-cta__btn cc-cta__btn--primary" href="${_esc(ph)}" data-track-cta="${_esc(pt)}">${_esc(pt)}</a>` : ''}
            ${st ? `<a class="cc-cta__btn cc-cta__btn--secondary" href="${_esc(sh)}" data-track-cta="${_esc(st)}">${_esc(st)}</a>` : ''}
          </div>
        </div>`;
