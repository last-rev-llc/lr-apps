(function () {
  ...
  class CcCard extends HTMLElement {
    static get observedAttributes() { return ['title', 'overline', 'subtitle', 'body', 'href', 'image', 'variant', 'layout']; }
    ...
    _render() {
      ...
      this.innerHTML = `<${tag} class="cc-card cc-card--${_esc(variant)} cc-card--${_esc(layout)}"${hrefAttr}>${imgHTML}${contentHTML}</${tag}>`;
