...
| `APP_SELF_ENROLL_SLUGS`               | empty (dev: all)                   | empty (free apps auto-enroll)            | empty (free apps auto-enroll)             | Override only — free-tier apps self-enroll via the registry; list non-free slugs here to expose them. |
