...
class CcContentful extends HTMLElement {
  ...
  async _load() {
    ...
    if (!src) return;
    try { this._data = await (await fetch(src)).json(); this._applyParams(); this._render(); } catch (e) { console.error('cc-contentful:', e); }
  }
  ...
  _render() {
    ...
    // Summary cards
    const cards = [
      { label: 'Total', value: s.total, icon: 'database', color: 'var(--text)' },
      { label: 'Published', value: s.published, icon: 'check-circle', color: 'var(--green)' },
      { label: 'Drafts', value: s.draft, icon: 'edit-3', color: 'var(--blue)' },
      { label: 'Changed', value: s.changed, icon: 'refresh-cw', color: 'var(--yellow)' },
      { label: 'Stale', value: s.stale, icon: 'alert-triangle', color: s.stale > 0 ? 'var(--red)' : 'var(--muted)' },
    ].map(c => `
      ...
      <tr>
        ...
        <td style="padding:8px 12px;">
          <a href="https://app.contentful.com/spaces/${e.space ? '' : ''}entries/${e.id}" target="_blank" rel="noopener"
             style="font-size:11px;color:#7c3aed;text-decoration:none;">Open ↗</a>
        </td>
      </tr>
    `).join('');
    ...
    const ctRows = ctData.map(c => `
      <tr>
        <td style="padding:6px 12px;font-size:13px;color:var(--text);cursor:pointer;text-decoration:underline;text-decoration-color:var(--border);"
            onclick="this.closest('cc-contentful')._setFilter('ct','${c.name}')">${this._esc(c.name)}</td>
    ...
    <div style="max-width:1200px;margin:0 auto;">
      ...
      <div style="background:var(--prompt-bg);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:24px;display:flex;align-items:center;gap:16px;">
        <i data-lucide="${this._trendIcon(v.trend)}" style="width:24px;height:24px;color:${this._trendColor(v.trend)};"></i>
        <div>
      ...
      <div style="display:flex;gap:12px;margin-bottom:20px;flex-wrap:wrap;align-items:center;">
        <cc-search placeholder="Search entries..." value="${this._esc(this._search || '')}" input-style="width:100%;padding:8px 12px;background:var(--bg);border:1px solid var(--border);border-radius:8px;color:var(--text);font-size:13px;outline:none;box-sizing:border-box;"></cc-search>
        <select onchange="this.closest('cc-contentful')._setFilter('space', this.value)"
          style="padding:8px 12px;background:var(--bg);border:1px solid ${space ? '#7c3aed' : 'var(--border)'};border-radius:8px;color:var(--text);font-size:13px;outline:none;font-weight:${space ? '600' : '400'};">
          <option value="">⚠ Select a Space</option>
          ${allSpaces.map(s => `<option value="${this._esc(s)}" ${space === s ? 'selected' : ''}>${this._esc(s)}</option>`).join('')}
        </select>
        <select onchange="this.closest('cc-contentful')._setFilter('ct', this.value)"
          ...
          ${contentTypes.map(c => `<option value="${c}" ${ct === c ? 'selected' : ''}>${this._esc(c)}</option>`).join('')}
        </select>
      ...
      <div style="background:var(--prompt-bg);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:24px;">
        <h3 style="margin:0 0 12px;font-size:14px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px;">
          <i data-lucide="alert-triangle" style="width:14px;height:14px;color:var(--yellow);"></i> Stale Drafts (${staleDrafts.length})
        </h3>
      ...
      <div style="background:var(--prompt-bg);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:24px;">
        <h3 style="margin:0 0 12px;font-size:14px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px;">
          <i data-lucide="check-circle" style="width:14px;height:14px;color:var(--green);"></i> Recent Publishes (${recentPubs.length})
        </h3>
        ${pubRows}
      </div>` : ''}
      ...
      <div style="background:var(--prompt-bg);border:1px solid var(--border);border-radius:12px;padding:16px;margin-bottom:24px;">
        <h3 style="margin:0 0 12px;font-size:14px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px;">
          <i data-lucide="layers" style="width:14px;height:14px;color:#7c3aed;"></i> By Content Type (${ctData.length})
        </h3>
        <div style="overflow-x:auto;">
          <table style="width:100%;border-collapse:collapse;">
            <thead><tr style="border-bottom:1px solid var(--border);">
