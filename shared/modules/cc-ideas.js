...
class CcIdeas extends HTMLElement {
  ...
  _render() {
    ...
    if (this._searchQuery) activeFilterCount++;
    if (this._activeCat !== 'All') activeFilterCount++;
    if (this._activeSource !== 'All') activeFilterCount++;
    if (this._activeApp !== 'All' && !this._appAttr) activeFilterCount++;
    if (this._activeShow !== 'active') activeFilterCount++;
    if (this._activeQuickFilter > 0) activeFilterCount++;
    ...
    this.innerHTML = `
      ...
      <div style="display:flex;gap:12px;align-items:center;margin-bottom:10px;justify-content:flex-end;">
        <cc-view-toggle app="ideas" value="${this._viewMode}"></cc-view-toggle>
        <cc-filter-drawer title="Filters" ${activeFilterCount ? 'active' : ''}>
      ...
      : this._viewMode === 'expanded' ? `<div class="view-expanded">${filtered.map(x => {
        const isCompleted = x.status === 'completed' || completed[x.id];
        return `<div class="expanded-card" data-cc-component="cc-idea-card" onclick="this.closest('cc-ideas')._openIdeaDetail('${x.id}')">
      ...
      : `<div class="idea-grid">${filtered.map(x => {
        ...
        return `
        <div class="idea-card${isHidden ? ' hidden-card' : ''}" id="card-${x.id}" data-cc-component="cc-idea-card" onclick="this.closest('cc-ideas')._openIdeaDetail('${x.id}')">
